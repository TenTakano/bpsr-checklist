...
const COUNTER_TASK_LABEL = upstreamTasksDocument.daily[2].label
  ...
  it('renders a column per character', () => {
    ...
    expect(
      screen.getByRole('button', {
        name: `${CHARACTER_NAME} ${upstreamTasksDocument.daily[0].label}`,
      }),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', {
        name: `${secondCharacter.name} ${upstreamTasksDocument.daily[0].label}`,
      }),
    ).toBeInTheDocument()
  })
})
...
describe('MatrixView / optional tasks', () => {
  it('marks optional task rows with a thin-display class and badge', () => {
    renderWithContext({ store: storeWithCharacter() })
    const row = screen.getByTitle(OPTIONAL_TASK.label).closest('tr')
